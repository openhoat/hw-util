...
util.optionCallbackArgParser = function (opt, cb) {
  ...
  result.cb = typeof result.cb === 'function' ? result.cb : util.noop;
  return result;
};
...
util.sendMail = function (opt, cb) {
  ...
  opt = args.opt;
  cb = args.cb;
  mailer = nodemailer.createTransport(opt.transport);
  return Promise.promisify(mailer.sendMail)
    .call(mailer, opt.mail)
    .nodeify(cb);
};


util.startHttpServer = function (opt, cb) {
  ...
  function echoHandler(req, res) {
    ...
    res.end(util.format('<html><head><title>Http Server</title></head><body><h1>Path</h1><h2>%s</h2></body></html>', req.url));
  }

  httpServer = http.createServer(opt.listener || echoHandler);
  return new Promise(
    function (resolve, reject) {
      ...
      if (typeof opt.port === 'number' && typeof opt.hostname === 'string') {
        httpServer.listen(opt.port, opt.hostname, resultHandler);
      } else if (typeof opt.path === 'string') {
        httpServer.listen(opt.path, resultHandler);
      } else {
        httpServer.listen(opt.port, resultHandler);
      }
    }).nodeify(cb);
};

util.stopHttpServer = function (opt, cb) {
  var Promise = require('bluebird')
    , args = util.optionCallbackArgParser.apply(null, arguments)
    , httpServer;
  opt = args.opt;
  ...
  return Promise.promisify(httpServer.close)
    .call(httpServer)
    .nodeify(cb);
};
...
util.uploadFtp = function (opt, cb) {
  var Promise = require('bluebird')
  ...
  return new Promise(function (resolve, reject) {
    ...
    ftpClient.on('ready', function () {
      ftpClient.put(opt.file.input, opt.file.destPath, function (err) {
        ftpClient.end();
        if (err) {
          return reject(err);
        }
