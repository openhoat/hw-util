...
util = {
  ...
  exec: function (cmd, args, options, callback, stdoutHandler, stderrHandler) {
    var childProcess = require('child_process')
      , command, child, process, out, err;
    args = util.parseArgs(arguments, [
      { name: 'cmd', optional: false, type: 'string' },
      { name: 'args', optional: true, type: 'object' },
      { name: 'options', optional: true, type: 'object' },
      { name: 'callback', optional: false, type: 'function', defaultValue: util.noop },
      { name: 'stdoutHandler', optional: true, type: 'function' },
      { name: 'stderrHandler', optional: true, type: 'function' }
    ]);
    args.callback = util.safeCallback(args.callback);
    if (args.args && !Array.isArray(args.args)) {
      args.args = [args.args];
    }
    console.log('args :', args);
    process = childProcess.spawn(args.cmd, args.args, args.options);
    if (args.stdoutHandler) {
      process.stdout.on('data', args.stdoutHandler);
    } else {
    ...
    if (args.stderrHandler) {
      process.stderr.on('data', args.stderrHandler);
    } else {
