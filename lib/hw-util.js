...
util = {
  ...
  compileConfig: function (config) {
    var configEnv = config.env || process.env['NODE_ENV'] || 'development';
    if (config.envs && config.envs[configEnv]) {
      util.copyProperties(config.envs[configEnv], config, false, ['envs']);
    }
    delete config.envs;
  },
  configToAbsolutePaths: function (config, baseDir, exclude, include) {
    var key;
    for (key in config) {
      if (config.hasOwnProperty(key)) {
        ...
        if (typeof config[key] === 'string') {
          config[key] = config[key].replace(/\.\//g, util.format('%s/', baseDir));
        } else if (typeof config[key] === 'object') {
          util.configToAbsolutePaths(config[key], baseDir, exclude, include);
        }
      }
    }
  },
  copyProperties: function (src, dest, onlyExisting, exclude, include, map) {
    var key, jsonString, clone;
    ...
    for (key in src) {
      if (src.hasOwnProperty(key)) {
        ...
        if (!onlyExisting || (!map && dest.hasOwnProperty(key)) || (map && dest.hasOwnProperty(map[key]))) {
          if (typeof src[key] === 'object' && src[key] !== null && ((!map && typeof dest[key] === 'object') || (map && typeof dest[map[key]] === 'object'))) {
            if (map && map[key]) {
              dest[map[key]] = util.copyProperties(src[key], dest[map[key]], onlyExisting);
            } else {
              dest[key] = util.copyProperties(src[key], dest[key], onlyExisting);
            }
          } else {
            try {
              jsonString = JSON.stringify(src[key]);
  ...
  loadConfig: function (options) {
    ...
    options = options || {};
    options.defaultConfigName = options.defaultConfigName || 'default';
    options.configDir = options.configDir || 'config';
    options.baseDir = options.baseDir || util.getCallerPath(options.depth);
    configAbsDir = path.join(options.baseDir, options.configDir);
    config = options.defaultConfig;
    try {
      if (config) {
        util.copyProperties(require(path.join(configAbsDir, options.defaultConfigName)), config, false);
      } else {
        config = require(path.join(configAbsDir, options.defaultConfigName));
      }
    } catch (err) {
      config = {};
    }
    ...
    files.forEach(function (file) {
      ...
      if (configFileBasename === options.defaultConfigName) {
        return;
      }
  ...
  loadConfig2: function (configDir, configName, depth, baseDir) {
    var path = require('path')
      , config, configAbsDir;
    configName = process.env['NODE_ENV'] || configName || 'default';
    configDir = configDir || 'config';
    baseDir = baseDir || util.getCallerPath(depth);
    configAbsDir = path.join(baseDir, configDir);
    try {
      config = require(path.join(configAbsDir, configName));
    } catch (err) {
      config = {};
    }
  ...
  parseArgs: function (args, specs) {
    var argsArray, result, spec, i, arg, j, k;
