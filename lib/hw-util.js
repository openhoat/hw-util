...
util = {
  ...
  date: {
    ...
    asString: function (date) {
      var format, vDay, vMonth, vYearLong, vYearShort, vYear, vHour, vMinute, vSecond, vMillisecond, vTimeZone, formatted;
      format = util.date.ISO8601_FORMAT;
      if (typeof(date) === 'string') {
        format = arguments[0];
        date = arguments[1];
      }
      vDay = util.date.addZero(date.getDate());
      vMonth = util.date.addZero(date.getMonth() + 1);
      vYearLong = util.date.addZero(date.getFullYear());
      vYearShort = util.date.addZero(date.getFullYear().toString().substring(2, 4));
      vYear = (format.indexOf("yyyy") > -1 ? vYearLong : vYearShort);
      vHour = util.date.addZero(date.getHours());
      vMinute = util.date.addZero(date.getMinutes());
      vSecond = util.date.addZero(date.getSeconds());
      vMillisecond = util.date.padWithZeros(date.getMilliseconds(), 3);
      vTimeZone = util.date.offset(date);
      formatted = format.
        replace(/dd/g, vDay).
        replace(/MM/g, vMonth).
